...
const Blog: React.FC = () => {
  ...
  const currentPost = blogPosts[parseInt(id as string || '')];

  if (id && !currentPost) {
    return (
      <Layout>
        <div className="container mx-auto px-4 py-8 text-center">Blog post not found</div>
      </Layout>
    );
  }
  ...
  return (
    <>
      <Head>
        <title>{id ? `${currentPost?.title} - Blog` : 'Blog'} - Perssonify</title>
        <meta name="description" content={id ? currentPost?.excerpt : "Stay updated with our latest insights, trends, and expert advice on digital marketing and business growth."} />
      </Head>
      <Layout>
